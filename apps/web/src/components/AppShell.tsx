...
import { AlertTriangle, ArrowLeftRight, CalendarClock, ChevronDown, Globe2, Landmark, Layers3, LayoutDashboard, LineChart, LogOut, Menu, Settings, WalletCards } from 'lucide-react'
...
export function AppShell() {
  ...
  const fundsLabel = isZh ? '国内基金' : 'China Funds'
  const reportingLabel = isZh ? '统一报告' : 'Reporting'
  const visibleNavItems = isDemoMode ? navItems.filter((item) => item.key !== 'reporting') : navItems

  const navLabel = (key: typeof navItems[number]['key']) => {
    if (key === 'contributions') return contributionsLabel
    if (key === 'funds') return fundsLabel
    if (key === 'reporting') return reportingLabel
    return t(`nav.${key}`)
  }
  ...
  const breadcrumb = title === 'funds' ? fundsLabel
    : title === 'reporting' ? reportingLabel
      : title === 'dashboard' ? t('nav.dashboard')
        : title === 'plan' ? t('nav.plan')
          : title === 'contributions' ? contributionsLabel
            : title === 'transactions' ? t('nav.transactions')
              : title === 'settings' ? t('nav.settings') : t('nav.etfs')
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
      ...
      <nav className="primary-nav" aria-label={t('common.primaryNavigation')}>
        ...
        {visibleNavItems.map(({ key, to, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `nav-link ${isActive ? 'nav-link-active' : ''}`} onClick={() => setMobileOpen(false)}><Icon size={17} strokeWidth={1.8} /><span>{navLabel(key)}</span>{key === 'plan' ? <span className="nav-pulse" /> : null}</NavLink>)}
      </nav>
    ...
    <div className="app-main">
      <header className="topbar">
        ...
        <div className="breadcrumbs"><span>DCA TERMINAL</span><span className="breadcrumb-separator">/</span><strong>{breadcrumb}</strong></div>
