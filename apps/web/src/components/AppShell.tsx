...
import { AlertTriangle, ArrowLeftRight, CalendarClock, ChevronDown, Globe2, Landmark, LayoutDashboard, LineChart, LogOut, Menu, Settings, WalletCards } from 'lucide-react'
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
      ...
      <nav className="primary-nav" aria-label={t('common.primaryNavigation')}>
        ...
        {navItems.map(({ key, to, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `nav-link ${isActive ? 'nav-link-active' : ''}`} onClick={() => setMobileOpen(false)}><Icon size={17} strokeWidth={1.8} /><span>{navLabel(key)}</span>{key === 'plan' ? <span className="nav-pulse" /> : null}</NavLink>)}
      </nav>
    ...
    <div className="app-main">
      <header className="topbar">
        ...
        <div className="breadcrumbs"><span>DCA TERMINAL</span><span className="breadcrumb-separator">/</span><strong>{title === 'funds' ? fundsLabel : title === 'dashboard' ? t('nav.dashboard') : title === 'plan' ? t('nav.plan') : title === 'contributions' ? contributionsLabel : title === 'transactions' ? t('nav.transactions') : title === 'settings' ? t('nav.settings') : t('nav.etfs')}</strong></div>
