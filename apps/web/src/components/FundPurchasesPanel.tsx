...
type PurchaseWithPendingNav = Omit<FundPurchase, 'nav' | 'shares'> & { nav: string | null; shares: string | null }
...
function copy(isZh: boolean) {
  return isZh ? {
    ...
    navHint: '可以买入当天直接保存。若该日 NAV 尚未公布，记录会先进入“待净值”状态；后续同步到该日净值后，系统自动确认成交 NAV 和份额。', feeHint: '小数口径，例如 0.0015 = 0.15%。买入金额按实际支付总额计算。', error: '操作失败', pendingNav: '待净值', pendingHint: '尚未确认份额；暂不计入持仓和收益，获取到买入日 NAV 后自动确认。',
  } : {
    ...
    navHint: 'You can save today’s purchase before NAV is published. It stays pending until the exact purchase-date NAV is synced, then NAV and shares are confirmed automatically.', feeHint: 'Decimal form, e.g. 0.0015 = 0.15%. The amount is treated as gross cash paid.', error: 'Operation failed', pendingNav: 'Pending NAV', pendingHint: 'Shares are not confirmed yet and are excluded from holdings/performance until purchase-date NAV arrives.',
  }
}
...
export function FundPurchasesPanel({ fund, isZh, locale }: { fund: MutualFund; isZh: boolean; locale: string }) {
  ...
  const rows = (purchases.data?.data ?? []) as PurchaseWithPendingNav[]
  ...
  return <>
    <Panel title={t.title} action={<button type="button" className="button button-primary button-small" onClick={() => openPurchase()}><Plus size={14} />{t.add}</button>}>
      ...
      {purchases.isLoading ? <LoadingBlock lines={3} /> : purchases.isError ? <ErrorState onRetry={() => void purchases.refetch()} /> : rows.length === 0 ? <EmptyState title={t.empty} action={<button type="button" className="button button-secondary button-small" onClick={() => openPurchase()}>{t.add}</button>} /> : <div className="data-table-wrap"><table className="data-table fund-purchase-table"><thead><tr><th>{t.date}</th><th>{t.amount}</th><th>{t.nav}</th><th>{t.fee}</th><th>{t.shares}</th><th>{t.notes}</th><th>{t.actions}</th></tr></thead><tbody>{rows.map((purchase) => {
        const pending = purchase.nav == null || purchase.shares == null
        return <tr key={purchase.id}><td><strong>{purchase.purchaseDate}</strong>{pending ? <small className="fund-purchase-pending">{t.pendingNav}</small> : null}</td><td>{money(purchase.grossAmount, locale)}</td><td>{pending ? '—' : Number(purchase.nav).toFixed(4)}</td><td>{money(purchase.purchaseFee, locale)} · {percent(purchase.purchaseFeeRate)}</td><td>{pending ? '—' : Number(purchase.shares).toFixed(4)}</td><td>{pending ? t.pendingHint : (purchase.notes ?? '—')}</td><td><div className="fund-purchase-actions"><button type="button" className="icon-button" title={t.edit} aria-label={t.edit} onClick={() => openPurchase(purchase)}><Edit3 size={14} /></button><button type="button" className="icon-button fund-purchase-delete" title={t.delete} aria-label={t.delete} onClick={() => setToDelete(purchase)}><Trash2 size={14} /></button></div></td></tr>
      })}</tbody></table></div>}
    </Panel>
