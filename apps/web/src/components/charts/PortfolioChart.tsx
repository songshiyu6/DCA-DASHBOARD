...
export function PortfolioChart({ data }: { data: PortfolioHistoryPoint[] }) {
  ...
  useEffect(() => {
    ...
    const points = data.filter((point) => Number.isFinite(Number(point.marketValue)) && Number.isFinite(Number(point.netInvested)))
    ...
    chart.setOption({
      ...
      series: [
        { name: 'Market value', type: 'line', smooth: 0.25, showSymbol: false, data: points.map((point) => Number(point.marketValue)), lineStyle: { width: 2, color: accent }, itemStyle: { color: accent }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(122,184,255,.16)' }, { offset: 1, color: 'rgba(122,184,255,0)' }]) } },
        { name: 'Net investment', type: 'line', smooth: 0.2, showSymbol: false, data: points.map((point) => Number(point.netInvested)), lineStyle: { width: 1.5, type: 'dashed', color: positive }, itemStyle: { color: positive } },
      ],
    })
