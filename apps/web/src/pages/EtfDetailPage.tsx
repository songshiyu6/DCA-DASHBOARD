...
export function EtfDetailPage() {
  ...
  const fallbackMessage = currentQuote?.quoteSession === 'REGULAR_FALLBACK'
    ? (isZh ? 'Yahoo 实时报价暂时不可用，当前显示常规交易时段价格。' : 'Yahoo live quote is temporarily unavailable; showing the regular-session price.')
    : quote.data?.meta.message
  const marketTimeLabel = isZh ? '行情时间' : 'Market time'
  const retrievedTimeLabel = isZh ? '抓取时间' : 'Retrieved'
  return <div className="page etf-detail-page">
    ...
    <DataStateBanner status={currentQuote?.status ?? quote.data?.meta.status ?? (quote.isError ? 'UNAVAILABLE' : 'STALE')} message={fallbackMessage} source={quote.data?.meta.source === 'FIXTURE' ? t('common.demoData') : quote.data?.meta.source} asOf={quote.data?.meta.asOf} retrievedAt={currentQuote?.retrievedAt ?? quote.data?.meta.retrievedAt} />
