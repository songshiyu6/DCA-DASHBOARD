...
import { decimal, formatCompactMoney, formatMoney, formatPercent, formatSignedPercent, formatTime } from '../lib/format'
...
function EtfCard({ instrument, quote, canMoveUp, canMoveDown, onMoveUp, onMoveDown, onOpen }: {
  ...
  quote?: Pick<Quote, 'price' | 'changePercent' | 'status' | 'quoteSession' | 'marketTimestamp'>
  canMoveUp: boolean
  canMoveDown: boolean
  onMoveUp: () => void
  onMoveDown: () => void
  ...
  const session = quote ? `${quoteSessionLabel(quote.quoteSession, isZh)} · ${formatTime(quote.marketTimestamp)} ET` : null
  return <article className="etf-card">
    ...
    <div className="etf-card-footer"><span>{instrument.exchange} · {instrument.currency}{session ? ` · ${session}` : ''}</span><span className="etf-card-footer-actions"><span className="etf-order-controls"><button type="button" className="icon-button etf-order-button" onClick={onMoveUp} disabled={!canMoveUp} aria-label={`Move ${instrument.symbol} up`} title={`Move ${instrument.symbol} up`}><ArrowUp size={14} /></button><button type="button" className="icon-button etf-order-button" onClick={onMoveDown} disabled={!canMoveDown} aria-label={`Move ${instrument.symbol} down`} title={`Move ${instrument.symbol} down`}><ArrowDown size={14} /></button></span>{quote ? <StatusBadge status={quote.status ?? 'UNAVAILABLE'} compact /> : <span>{t('common.loading')}</span>}</span></div>
  </article>
}
...
export function EtfsPage() {
  ...
  const quotes = useQueries({ queries: (instruments.data?.data ?? []).map((instrument) => ({
    queryKey: queryKeys.quote(instrument.symbol),
    queryFn: () => api.getQuote(instrument.symbol),
    staleTime: 60_000,
