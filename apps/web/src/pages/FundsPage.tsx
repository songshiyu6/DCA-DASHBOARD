...
import { AlertTriangle, CalendarDays, ChevronDown, ChevronRight, Edit3, Plus, RefreshCw, X } from 'lucide-react'
...
function copy(isZh: boolean) {
  return isZh ? {
    ...
    addFund: '添加基金', funds: '基金', noFunds: '还没有国内基金', noFundsHint: '先添加一个 6 位基金代码，再同步净值和交易日。',
    latestNav: '最新净值', navDate: '净值日期', gaps: '缺失净值', openDays: '开放日', sync: '同步 NAV', syncing: '同步中…', edit: '编辑',
    ...
    addFund: 'Add fund', funds: 'Funds', noFunds: 'No China funds yet', noFundsHint: 'Add a six-digit fund code, then sync NAV and trading dates.',
    latestNav: 'Latest NAV', navDate: 'NAV date', gaps: 'Missing NAV', openDays: 'Open days', sync: 'Sync NAV', syncing: 'Syncing…', edit: 'Edit',
...
export function FundsPage() {
  ...
  const [fundForm, setFundForm] = useState({ code: '', name: '', managementFeeRate: '0', confirmationTradingDays: '1', shareClass: '' })
  ...
  const syncFund = useMutation({
    mutationFn: (id: string) => api.syncFund(id),
  ...
  const openFund = (fund?: MutualFund) => {
    ...
    } : { code: '', name: '', managementFeeRate: '0', confirmationTradingDays: '1', shareClass: '' })
  ...
  return <div className="page funds-page">
    ...
    {funds.isLoading ? <Panel><LoadingBlock lines={6} /></Panel> : funds.isError ? <ErrorState onRetry={() => void funds.refetch()} /> : (funds.data?.data.length ?? 0) === 0 ? <Panel><EmptyState title={t.noFunds} action={<button type="button" className="button button-primary" onClick={() => openFund()}><Plus size={15} />{t.addFund}</button>} /><p className="fund-empty-hint">{t.noFundsHint}</p></Panel> : <div className="fund-workspace">
      ...
      <div className="fund-detail-stack">{selectedFund ? <>
        <Panel title={`${selectedFund.code} · ${selectedFund.name}`} detail={`${t.selectedFund} · CNY`} action={<div className="fund-panel-actions"><button type="button" className="button button-secondary button-small" onClick={() => openFund(selectedFund)}><Edit3 size={14} />{t.edit}</button><button type="button" className="button button-secondary button-small" disabled={syncFund.isPending} onClick={() => syncFund.mutate(selectedFund.id)}><RefreshCw size={14} className={syncFund.isPending ? 'spin' : ''} />{syncFund.isPending ? t.syncing : t.sync}</button></div>}>
    ...
    {fundDialogOpen ? <Dialog labelledBy="fund-dialog-title" onClose={() => setFundDialogOpen(false)}><button type="button" className="modal-close icon-button" onClick={() => setFundDialogOpen(false)} aria-label={t.cancel}><X size={17} /></button><h2 id="fund-dialog-title">{editingFund ? t.fundDialogEdit : t.fundDialogCreate}</h2><form className="fund-form" onSubmit={submitFund}><label><span>{t.code}</span><input required maxLength={16} value={fundForm.code} onChange={(event) => setFundForm({ ...fundForm, code: event.target.value })} placeholder="000001" /></label><label><span>{t.name}</span><input required maxLength={255} value={fundForm.name} onChange={(event) => setFundForm({ ...fundForm, name: event.target.value })} /></label><label><span>{t.managementFee}</span><input required inputMode="decimal" value={fundForm.managementFeeRate} onChange={(event) => setFundForm({ ...fundForm, managementFeeRate: event.target.value })} /><small>{t.managementFeeHint}</small></label><label><span>{t.confirmationDays}</span><input required type="number" min="0" max="10" step="1" value={fundForm.confirmationTradingDays} onChange={(event) => setFundForm({ ...fundForm, confirmationTradingDays: event.target.value })} /></label><label><span>{t.shareClass}</span><input maxLength={16} value={fundForm.shareClass} onChange={(event) => setFundForm({ ...fundForm, shareClass: event.target.value })} placeholder="A / C" /></label><div className="fund-form-actions"><button type="button" className="button button-secondary" onClick={() => setFundDialogOpen(false)}>{t.cancel}</button><button type="submit" className="button button-primary" disabled={saveFund.isPending}>{t.save}</button></div></form></Dialog> : null}
