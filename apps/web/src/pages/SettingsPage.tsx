...
function persistTimezones(marketTimezone?: AppTimezone, displayTimezone?: AppTimezone): void {
  localStorage.setItem('dca-market-timezone', marketTimezone ?? DEFAULT_MARKET_TIMEZONE)
  localStorage.setItem('dca-display-timezone', displayTimezone ?? DEFAULT_DISPLAY_TIMEZONE)
}
...
export function SettingsPage() {
  ...
  const save = useMutation({ mutationFn: (patch: Partial<AppSettings>) => api.updateSettings(patch), onSuccess: (result) => { setDraft(result.data); applyTheme(result.data.theme); persistTimezones(result.data.marketTimezone, result.data.displayTimezone); void queryClient.invalidateQueries({ queryKey: queryKeys.settings }) } })
  useEffect(() => { if (settings.data) { setDraft(settings.data.data); applyTheme(settings.data.data.theme); persistTimezones(settings.data.data.marketTimezone, settings.data.data.displayTimezone) } }, [settings.data])
