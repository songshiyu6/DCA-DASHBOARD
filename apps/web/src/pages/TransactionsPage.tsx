...
const initialForm: TransactionFormValues = { tradeDate: '', transactionType: 'BUY', instrumentSymbol: 'VOO', quantity: '', unitPrice: '', amount: '', fee: '0.00', notes: '', planCycleId: '', contributionType: 'UNPLANNED' }
...
function transactionValues(transaction?: Transaction): TransactionFormValues {
  if (!transaction) return { ...initialForm, tradeDate: localDateInputValue() }
  return {
...
export function TransactionsPage() {
  ...
  const transactionPlan = planItems.find((plan) => plan.status === 'ACTIVE') ?? [...planItems].reverse().find((plan) => plan.status === 'PAUSED')
  const cycles = useQuery({ queryKey: transactionPlan ? queryKeys.transactionCycles(transactionPlan.id) : queryKeys.transactionCycles('none'), queryFn: () => api.getCycles(transactionPlan?.id ?? ''), enabled: Boolean(transactionPlan) })
  const remove = useMutation({ mutationFn: (id: string) => api.deleteTransaction(id), onSuccess: () => { void invalidateTransactionQueries(queryClient, transactionPlan?.id) } })
  ...
  const modalCycles = cycles.data?.data ?? transactionPlan?.cycles ?? []
  return <div className="page transactions-page">
    ...
    {addOpen ? <TransactionModal transaction={editing ?? undefined} cycles={modalCycles} plan={transactionPlan} onClose={() => { setAddOpen(false); setEditing(null) }} onSaved={() => { setAddOpen(false); setEditing(null) }} /> : null}{csvOpen ? <CsvModal planId={transactionPlan?.id} onClose={() => setCsvOpen(false)} onImported={() => setCsvOpen(false)} /> : null}
  </div>
}
