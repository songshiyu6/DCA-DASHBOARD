...
function TransactionModal({ transaction, cycles, planId, onClose, onSaved }: { transaction?: Transaction; cycles: PlanCycle[]; planId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  const save = useMutation({ mutationFn: (values: TransactionFormValues) => transaction ? api.updateTransaction(transaction.id, transactionInput(values)) : api.createTransaction(transactionInput(values)), onSuccess: () => { void invalidateTransactionQueries(queryClient, planId); onSaved() } })
  ...
  return <Dialog className="modal-form" labelledBy="transaction-modal-title" describedBy="transaction-modal-description" onClose={onClose}><button type="button" className="modal-close icon-button" onClick={onClose} aria-label={t('common.close')}><X size={17} /></button><span className="modal-icon modal-icon-blue"><ArrowDownLeft size={18} /></span><h2 id="transaction-modal-title">{transaction ? t('transactions.editTransaction') : t('transactions.addTransaction')}</h2><p id="transaction-modal-description">{t('transactions.formHint')}</p><form onSubmit={(event) => { void form.handleSubmit((values) => save.mutate(values))(event) }} className="form-stack">
    ...
    <div className="form-footer"><button type="button" className="button button-ghost" onClick={onClose}>{t('common.cancel')}</button><button type="submit" className="button button-primary" disabled={save.isPending}><Plus size={15} />{save.isPending ? t('transactions.saving') : transaction ? t('common.save') : t('transactions.saveTransaction')}</button></div>
  </form></Dialog>
}
...
export function TransactionsPage() {
  ...
  return <div className="page transactions-page">
    ...
    {addOpen ? <TransactionModal transaction={editing ?? undefined} cycles={modalCycles} planId={activePlan?.id} onClose={() => { setAddOpen(false); setEditing(null) }} onSaved={() => { setAddOpen(false); setEditing(null) }} /> : null}{csvOpen ? <CsvModal planId={activePlan?.id} onClose={() => setCsvOpen(false)} onImported={() => setCsvOpen(false)} /> : null}
  </div>
}
